 ...
 *
 *  Modified: 14-Mar-2001 (032WE2001 Edition)
 *
 ...
 * History :
 *
 * 28-Jul-2001 (Tim Baumgarten) : Changed "var pDestinationSurface : IDirect3DSurface8" to "const pDestinationSurface : IDirect3DSurface8" in IDirect3DDevice8.CopyRects
 * 14-Mar-2001 (Tim Baumgarten) : Changed CreateVertexShader as pFunction can be nil
 ...
 * 14-Dec-2000 (Tim Baumgarten) : Changed some parameters of IDirect3DDevice8.DrawRectPatch and IDirect3DDevice8.DrawTriPatch to Pointers.
 *                              : Added versions without underlines of some structures
 *                              : Added "Pointer to Structure" (PStructure = ^TStructure) to all structures.
 * 26-Nov-2000 (Tim Baumgarten) : Returncodes are now typecasted with HResult
 *
 ***************************************************************************)

unit se_DirectXGraphics;
...
type
  ...
  TD3DLight8 = packed record
    _Type        : LongWord;        (* Type of light source *) //D3DLIGHTTYPE
...
type
  TD3DTransformStateType = LongWord;
...
const
  ...
  function D3DTS_WORLDMATRIX(index : LongWord) : LongWord; // (D3DTRANSFORMSTATETYPE)(index + 256)
...
type
  TD3DMaterialColorSource = LongWord;
 ...
 *)
type
  TD3DTextureStageStateType = LongWord;
 ...
 *)
  ...
  D3DTA_TFACTOR           = $00000003;  // select RENDERSTATE_TEXTUREFACTOR (read only)
...
//
type
   TD3DTextureFilterType = LongWord;
...
const
  ...
  D3DVSD_TOKEN_CONSTMEM    = 4;          // constant memory from shader
  ...
  function D3DVSD_MAKETOKENTYPE(tokenType : LongWord) : LongWord;
...
//
  function D3DVSD_STREAM(_StreamNumber : LongWord) : LongWord;
...
const
  ...
  D3DVSD_STREAM_TESS = ((D3DVSD_TOKEN_STREAM shl D3DVSD_TOKENTYPESHIFT) and D3DVSD_TOKENTYPEMASK) or D3DVSD_STREAMTESSMASK;
...
// _Type [D3DVSDT_*] dimensionality and arithmetic data type
  ...
  function D3DVSD_REG( _VertexRegister, _Type : LongWord) : LongWord;

// Skip _DWORDCount DWORDs in vertex
//
  function D3DVSD_SKIP(_DWORDCount : LongWord) : LongWord;
...
//
  function D3DVSD_CONST( _ConstantAddress, _Count : LongWord) : LongWord;
...
//
  function D3DVSD_TESSUV( _VertexRegister : LongWord) : LongWord;
...
type
  TD3DShader_Param_Register_Type = LongWord;
...
//
type
  TD3DVS_RastOut_Offsets = LongWord;
...
type
  TD3DShader_Param_SRCMod_Type = LongWord;
...
type
  TD3DTextureTransformFlags = LongWord;
...
type
  TD3DMultiSample_Type = LongWord;
  TD3DMultiSampleType = TD3DMultiSample_Type;
...
(* Back buffer types *)
type
  TD3DBackBuffer_Type = LongWord;
  TD3DBackBufferType = LongWord;
...
type
  TD3DCubeMap_Faces = LongWord;
  TD3DCubeMapFaces = TD3DCubeMap_Faces;
...
var
  _Direct3DCreate8 : function (SDKVersion : Cardinal) : Pointer; stdcall;

function DXGErrorString(Value: HResult) : string;
...
function D3DTS_WORLDMATRIX(index : LongWord) : LongWord;
begin
 Result := index + 256;
end;
...
function D3DVSD_MAKETOKENTYPE(tokenType : LongWord) : LongWord;
begin
  Result := (tokenType shl D3DVSD_TOKENTYPESHIFT) and D3DVSD_TOKENTYPEMASK;
end;
...
begin
  ...
  begin
    ...
    if D3D8DLL <> 0 then _Direct3DCreate8 := GetProcAddress(D3D8DLL, 'Direct3DCreate8');
  end;
end;
