...
uses
  ...
  se_DXClass, dibimage, se_DXTexImg, se_DirectX;
...
type
  ...
  public
    ...
    property OnRestoreSurface: TNotifyEvent read FOnRestoreSurface write FOnRestoreSurface;
    property Image: TDXTextureImage read FImage;
  end;
...
uses
  se_DXConsts, se_DXRender;

function DXDirectDrawEnumerate(lpCallback: TDDEnumCallbackA; lpContext: Pointer): HRESULT;
type
  TDirectDrawEnumerate = function(lpCallback: TDDEnumCallbackA; lpContext: Pointer): HRESULT; stdcall;
begin
...
type
  ...
  var
    ...
    begin
      ...
      rec.D3DFlag := False;
      if Direct3D <> nil then Direct3D.EnumDevices(@EnumDeviceCallBack, rec);
      Result := rec.D3DFlag;
...
const
  ...
  procedure SetPixel1(const ddsd: TDDSurfaceDesc2; x, y: Integer; c: DWORD);
  var yy: integer;
  begin
    yy := Integer(ddsd.lpSurface)+ddsd.lPitch*y + x div 8;
    PByte(yy)^ :=
      (PByte(yy)^ and (not Mask1[x mod 8])) or (c shl Shift1[x mod 8]);
  end;

  procedure SetPixel2(const ddsd: TDDSurfaceDesc2; x, y: Integer; c: DWORD);
  var yy: integer;
  begin
    yy := Integer(ddsd.lpSurface)+ddsd.lPitch*y + x div 4;
    PByte(yy)^ :=
      (PByte(yy)^ and (not Mask2[x mod 4])) or (c shl Shift2[x mod 4]);
  end;

  procedure SetPixel4(const ddsd: TDDSurfaceDesc2; x, y: Integer; c: DWORD);
  var yy: integer;
  begin
    yy := Integer(ddsd.lpSurface)+ddsd.lPitch*y + x div 2;
    PByte(yy)^ :=
      (PByte(yy)^ and (not Mask4[x mod 2])) or (c shl Shift4[x mod 2]);
  end;

  procedure SetPixel8(const ddsd: TDDSurfaceDesc2; x, y: Integer; c: DWORD);
  var yy: integer;
  begin
    yy := Integer(ddsd.lpSurface)+ddsd.lPitch*y + x;
    PByte(yy)^ := c;
  end;

  procedure SetPixel16(const ddsd: TDDSurfaceDesc2; x, y: Integer; c: DWORD);
  var yy: integer;
  begin
    yy := Integer(ddsd.lpSurface)+ddsd.lPitch*y + x * 2;
    PWord(yy)^ := c;
  end;

  procedure SetPixel24(const ddsd: TDDSurfaceDesc2; x, y: Integer; c: DWORD);
  var yy: integer;
  begin
    yy := Integer(ddsd.lpSurface)+ddsd.lPitch*y + x * 3;
    PByte(yy)^ := c shr 0;
  ...
  procedure SetPixel32(const ddsd: TDDSurfaceDesc2; x, y: Integer; c: DWORD);
  var yy: integer;
  begin
    yy := Integer(ddsd.lpSurface)+ddsd.lPitch*y + x * 4;
    PDWord(yy)^ := c;
  end;
...
//  var yy: integer;
  begin
    case ddsd.ddpfPixelFormat.dwRGBBitCount of
      1: SetPixel1(ddsd, x, y, c);
      2: SetPixel2(ddsd, x, y, c);
      4: SetPixel4(ddsd, x, y, c);
      8: SetPixel8(ddsd, x, y, c);
     16: SetPixel16(ddsd, x, y, c);
     24: SetPixel24(ddsd, x, y, c);
     32: SetPixel32(ddsd, x, y, c);
    end;
{    yy := Integer(ddsd.lpSurface)+ddsd.lPitch*y;
    case ddsd.ddpfPixelFormat.dwRGBBitCount of
      1 : PByte(yy+x div 8)^ :=
            (PByte(yy+x div 8)^ and (not Mask1[x mod 8])) or (c shl Shift1[x mod 8]);
      2 : PByte(yy+x div 4)^ :=
            (PByte(yy+x div 4)^ and (not Mask2[x mod 4])) or (c shl Shift2[x mod 4]);
      4 : PByte(yy+x div 2)^ :=
            (PByte(yy+x div 2)^ and (not Mask4[x mod 2])) or (c shl Shift4[x mod 2]);
      8 : PByte(yy+x)^ := c;
      16: PWord(yy+x*2)^ := c;
      24: begin
            yy := yy+x*3;
...
{    case ddsd.ddpfPixelFormat.dwRGBBitCount of
  ...
  var
    ...
    x, y: Integer;
    ddsddwWidth: integer;
    ddsddwHeight: integer;
    linesize: integer;
  begin
    ddsd.dwSize := SizeOf(ddsd);
    if Dest.Lock(nil, ddsd, DDLOCK_WAIT, 0)=0 then
    begin
      ddsddwWidth := ddsd.dwWidth - 1;
      ddsddwHeight := ddsd.dwHeight - 1;
      try
        ...
        begin
          linesize := (Integer(ddsd.dwWidth)*SrcImage.BitCount+7) div 8;
          for y := 0 to ddsddwHeight do
            Move(SrcImage.ScanLine[y]^, Pointer(Integer(ddsd.lpSurface)+ddsd.lPitch*y)^, linesize);
        end else
        begin
          for y := 0 to ddsddwHeight do
          begin
            for x := 0 to ddsddwWidth do
              SetPixel(ddsd, x, y, dxtDecodeChannel(SrcImage.idx_index, SrcImage.GetPixelFast(x, y)));
          end;
        end;
      finally
  ...
  var
    ...
    pidx_index: ^TDXTextureImageChannel;
    topBits: Pointer;
    ddsddwWidth: integer;
    ddsddwHeight: integer;
    ppalentries: PDXTextureImage_PaletteEntries;
  begin
    ddsd.dwSize := SizeOf(ddsd);
    if Dest.Lock(nil, ddsd, DDLOCK_WAIT, 0)=0 then
    begin
      ...
      ddsddwHeight := ddsd.dwHeight - 1;
      try
        ppalentries := @SrcImage.idx_palette;
        ...
        begin
          for y := 0 to ddsddwHeight do
            for x := 0 to ddsddwWidth do
            begin
              c := SrcImage.GetPixelFast(x, y);
              cIdx := dxtDecodeChannel(SrcImage.idx_index, c);

              pEntry := @ppalentries[cIdx];
              c := dxtEncodeChannel(dest_red_fmt, pEntry.peRed) or
                dxtEncodeChannel(dest_green_fmt, pEntry.peGreen) or
                dxtEncodeChannel(dest_blue_fmt, pEntry.peBlue) or
             ...
             1: begin
                  for y := 0 to ddsddwHeight do
                    ...
                    begin
                      ...
                      c := dxtEncodeChannel(dest_red_fmt, pEntry.peRed) or
                        dxtEncodeChannel(dest_green_fmt, pEntry.peGreen) or
                        dxtEncodeChannel(dest_blue_fmt, pEntry.peBlue) or cA;

                      SetPixel1(ddsd, x, y, c);
                    end;
                end;
             ...
             4: begin
                  for y := 0 to ddsddwHeight do
                    ...
                    begin
                      ...
                      SetPixel4(ddsd, x, y, c);
                    end;
                end;

             8: begin
                  pidx_index := @SrcImage.idx_index;
                  for y := 0 to ddsddwHeight do
                  begin
                    yy := Integer(ddsd.lpSurface) + ddsd.lPitch * y;
                    for x := 0 to ddsddwWidth do
                    begin
                      c := SrcImage.GetPixelFast(x, y);
//                      cIdx := dxtDecodeChannel(SrcImage.idx_index, c);
                      cIdx := ((c  and pidx_index.Mask) shr pidx_index._rshift) shl pidx_index._lshift;
                      cIdx := cIdx or (cIdx shr pidx_index._BitCount2);

                      pEntry := @ppalentries[cIdx];

                      PByte(yy)^ :=
                        dxtEncodeChannel(dest_red_fmt, pEntry.peRed) or
                        dxtEncodeChannel(dest_green_fmt, pEntry.peGreen) or
                        dxtEncodeChannel(dest_blue_fmt, pEntry.peBlue) or cA;

                      inc(yy);
//                      SetPixel8(ddsd, x, y, c);
                    end;
                  end;
                end;
            ...
            16: begin
                  pidx_index := @SrcImage.idx_index;
                  case SrcImage.BitCount of
                     8: begin
                          if (pidx_index.Mask = 255) and (pidx_index._rshift = 0) and
                             (pidx_index._lshift = 0) and (pidx_index._BitCount2 = 0) then
                          begin
                            for y := 0 to ddsddwHeight do
                            begin
                              topBits := Pointer(integer(SrcImage.TopPBits) + SrcImage.NextLine * y) ;
                              for x := 0 to ddsddwWidth do
                              begin
                                c := PByte(topBits)^; // SrcImage.GetPixelFast8(x, y);
                                pEntry := @ppalentries[c];

                                c := dxtEncodeChannel(dest_red_fmt, pEntry.peRed) or
                                  dxtEncodeChannel(dest_green_fmt, pEntry.peGreen) or
                                  dxtEncodeChannel(dest_blue_fmt, pEntry.peBlue);

                                SetPixel16(ddsd, x, y, c);
                          ...
                          begin
                            ...
                            begin
                              ...
                              begin
                                c := PByte(topBits)^; // SrcImage.GetPixelFast8(x, y);
                                cIdx := dxtDecodeChannel(pidx_index^, c);
                                pEntry := @ppalentries[cIdx];
                  ...
                  case SrcImage.BitCount of
                     8: begin
                          ...
                          begin
                            ...
                            if (dest_red_fmt.Mask = $FF0000) and
                               (dest_green_fmt.Mask = $FF00) and
                               (dest_blue_fmt.Mask = $FF) and
                               (dest_blue_fmt._lshift = 0) and
                               (dest_green_fmt._lshift = 0) and
                               (dest_red_fmt._lshift = 0) and
                               (dest_blue_fmt._rshift = 0) and
                               (dest_red_fmt._rshift = 16) and
                               (dest_green_fmt._rshift = 8) then
                            begin

                              SrcImage.MakeCommonPalette;

                              ppalentries := @SrcImage.idx_CommonPalette;

                              if ddsddwWidth = 63 then
                              begin
                                for y := 0 to ddsddwHeight do
                                begin
                                  yy := Integer(ddsd.lpSurface) + ddsd.lPitch * y;
                                  topBits := Pointer(integer(SrcImage.TopPBits) + SrcImage.NextLine * y);
                                    {$I ddsd32_SrcImage8_fastloop.inc} //001
                                    {$I ddsd32_SrcImage8_fastloop.inc} //002
                                    {$I ddsd32_SrcImage8_fastloop.inc} //003
                                    {$I ddsd32_SrcImage8_fastloop.inc} //004
                                    {$I ddsd32_SrcImage8_fastloop.inc} //005
                                    {$I ddsd32_SrcImage8_fastloop.inc} //006
                                    {$I ddsd32_SrcImage8_fastloop.inc} //007
                                    {$I ddsd32_SrcImage8_fastloop.inc} //008
                                    {$I ddsd32_SrcImage8_fastloop.inc} //009
                                    {$I ddsd32_SrcImage8_fastloop.inc} //010
                                    {$I ddsd32_SrcImage8_fastloop.inc} //011
                                    {$I ddsd32_SrcImage8_fastloop.inc} //012
                                    {$I ddsd32_SrcImage8_fastloop.inc} //013
                                    {$I ddsd32_SrcImage8_fastloop.inc} //014
                                    {$I ddsd32_SrcImage8_fastloop.inc} //015
                                    {$I ddsd32_SrcImage8_fastloop.inc} //016
                                    {$I ddsd32_SrcImage8_fastloop.inc} //017
                                    {$I ddsd32_SrcImage8_fastloop.inc} //018
                                    {$I ddsd32_SrcImage8_fastloop.inc} //019
                                    {$I ddsd32_SrcImage8_fastloop.inc} //020
                                    {$I ddsd32_SrcImage8_fastloop.inc} //021
                                    {$I ddsd32_SrcImage8_fastloop.inc} //022
                                    {$I ddsd32_SrcImage8_fastloop.inc} //023
                                    {$I ddsd32_SrcImage8_fastloop.inc} //024
                                    {$I ddsd32_SrcImage8_fastloop.inc} //025
                                    {$I ddsd32_SrcImage8_fastloop.inc} //026
                                    {$I ddsd32_SrcImage8_fastloop.inc} //027
                                    {$I ddsd32_SrcImage8_fastloop.inc} //028
                                    {$I ddsd32_SrcImage8_fastloop.inc} //029
                                    {$I ddsd32_SrcImage8_fastloop.inc} //030
                                    {$I ddsd32_SrcImage8_fastloop.inc} //031
                                    {$I ddsd32_SrcImage8_fastloop.inc} //032
                                    {$I ddsd32_SrcImage8_fastloop.inc} //033
                                    {$I ddsd32_SrcImage8_fastloop.inc} //034
                                    {$I ddsd32_SrcImage8_fastloop.inc} //035
                                    {$I ddsd32_SrcImage8_fastloop.inc} //036
                                    {$I ddsd32_SrcImage8_fastloop.inc} //037
                                    {$I ddsd32_SrcImage8_fastloop.inc} //038
                                    {$I ddsd32_SrcImage8_fastloop.inc} //039
                                    {$I ddsd32_SrcImage8_fastloop.inc} //040
                                    {$I ddsd32_SrcImage8_fastloop.inc} //041
                                    {$I ddsd32_SrcImage8_fastloop.inc} //042
                                    {$I ddsd32_SrcImage8_fastloop.inc} //043
                                    {$I ddsd32_SrcImage8_fastloop.inc} //044
                                    {$I ddsd32_SrcImage8_fastloop.inc} //045
                                    {$I ddsd32_SrcImage8_fastloop.inc} //046
                                    {$I ddsd32_SrcImage8_fastloop.inc} //047
                                    {$I ddsd32_SrcImage8_fastloop.inc} //048
                                    {$I ddsd32_SrcImage8_fastloop.inc} //049
                                    {$I ddsd32_SrcImage8_fastloop.inc} //050
                                    {$I ddsd32_SrcImage8_fastloop.inc} //051
                                    {$I ddsd32_SrcImage8_fastloop.inc} //052
                                    {$I ddsd32_SrcImage8_fastloop.inc} //053
                                    {$I ddsd32_SrcImage8_fastloop.inc} //054
                                    {$I ddsd32_SrcImage8_fastloop.inc} //055
                                    {$I ddsd32_SrcImage8_fastloop.inc} //056
                                    {$I ddsd32_SrcImage8_fastloop.inc} //057
                                    {$I ddsd32_SrcImage8_fastloop.inc} //058
                                    {$I ddsd32_SrcImage8_fastloop.inc} //059
                                    {$I ddsd32_SrcImage8_fastloop.inc} //060
                                    {$I ddsd32_SrcImage8_fastloop.inc} //061
                                    {$I ddsd32_SrcImage8_fastloop.inc} //062
                                    {$I ddsd32_SrcImage8_fastloop.inc} //063
                                    {$I ddsd32_SrcImage8_fastloop_end.inc} //064
                                end;

                              end
                              else if ddsddwWidth = 127 then
                              begin
                                for y := 0 to ddsddwHeight do
                                begin
                                  yy := Integer(ddsd.lpSurface) + ddsd.lPitch * y;
                                  topBits := Pointer(integer(SrcImage.TopPBits) + SrcImage.NextLine * y);
                                    ...
                                    {$I ddsd32_SrcImage8_fastloop.inc} //062
                                    {$I ddsd32_SrcImage8_fastloop.inc} //063
                                    {$I ddsd32_SrcImage8_fastloop.inc} //064
                              ...
                              begin
                                ...
                                begin
                                  ...
                                  topBits := Pointer(integer(SrcImage.TopPBits) + SrcImage.NextLine * y);
                                    ...
                                    {$I ddsd32_SrcImage8_fastloop.inc} //030
                                    {$I ddsd32_SrcImage8_fastloop.inc} //031
                                    {$I ddsd32_SrcImage8_fastloop_end.inc} //032
                                end;
                              end

                              else if ddsddwWidth = 255 then
                              begin
                                for y := 0 to ddsddwHeight do
                                begin
                                  yy := Integer(ddsd.lpSurface) + ddsd.lPitch * y;
                              ...
                              begin
                                ...
                                begin
                                  ...
                                  for x := ddsddwWidth downto 0 do
                                  begin
                                    {$I ddsd32_SrcImage8_fastloop.inc}
                                  end;
                            ...
                            begin
                              ...
                              begin
                                ...
                                begin
                                  ...
                                  PDWord(yy)^ :=
                                    (((pEntry.peRed shl dest_red_fmt._rshift) shr dest_red_fmt._lshift) and dest_red_fmt.Mask) or
                                    (((pEntry.peGreen shl dest_green_fmt._rshift) shr dest_green_fmt._lshift) and dest_green_fmt.Mask) or
                                    (((pEntry.peBlue shl dest_blue_fmt._rshift) shr dest_blue_fmt._lshift) and dest_blue_fmt.Mask);
                            ...
                            begin
                              ...
                              begin
                                ...
                                PDWord(yy)^ :=
                                  (((pEntry.peRed shl dest_red_fmt._rshift) shr dest_red_fmt._lshift) and dest_red_fmt.Mask) or
                                  (((pEntry.peGreen shl dest_green_fmt._rshift) shr dest_green_fmt._lshift) and dest_green_fmt.Mask) or
                                  (((pEntry.peBlue shl dest_blue_fmt._rshift) shr dest_blue_fmt._lshift) and dest_blue_fmt.Mask) or cA;
                      ...
                      begin
                        ...
                        begin
                          ...
                          PDWord(yy)^ :=
                            (((pEntry.peRed shl dest_red_fmt._rshift) shr dest_red_fmt._lshift) and dest_red_fmt.Mask) or
                            (((pEntry.peGreen shl dest_green_fmt._rshift) shr dest_green_fmt._lshift) and dest_green_fmt.Mask) or
                            (((pEntry.peBlue shl dest_blue_fmt._rshift) shr dest_blue_fmt._lshift) and dest_blue_fmt.Mask) or
                            cA;
             ...
             8: begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      PByte(yy)^ :=
                        dxtEncodeChannel(dest_red_fmt, dxtDecodeChannel(SrcImage.rgb_red, c)) or
                        dxtEncodeChannel(dest_green_fmt, dxtDecodeChannel(SrcImage.rgb_green, c)) or
                        dxtEncodeChannel(dest_blue_fmt, dxtDecodeChannel(SrcImage.rgb_blue, c)) or cA;
            ...
            32: begin
                  case SrcImage.BitCount of
                     8: begin
                          ...
                          begin
                            ...
                            begin
                              ...
                              for x := 0 to ddsddwWidth do
                              begin
                                c := SrcImage.GetPixelFast8(x, y);

                                PDWord(yy)^ :=
                                  dxtEncodeChannel(dest_red_fmt, dxtDecodeChannel(SrcImage.rgb_red, c)) or
                                  dxtEncodeChannel(dest_green_fmt, dxtDecodeChannel(SrcImage.rgb_green, c)) or
                                  dxtEncodeChannel(dest_blue_fmt, dxtDecodeChannel(SrcImage.rgb_blue, c));
                    ...
                    24: begin
                          ...
                          begin
                            c := $0AF030;
                            if dxtEncodeChannel(dest_red_fmt, dxtDecodeChannel(SrcImage.rgb_red, c)) or
                               dxtEncodeChannel(dest_green_fmt, dxtDecodeChannel(SrcImage.rgb_green, c)) or
                               dxtEncodeChannel(dest_blue_fmt, dxtDecodeChannel(SrcImage.rgb_blue, c)) = c then
                            begin
                              for y := 0 to ddsddwHeight do
                              begin
                                yy := Integer(ddsd.lpSurface) + ddsd.lPitch * y;
                                for x := 0 to ddsddwWidth do
                                begin
                                  PDWord(yy)^ := SrcImage.GetPixelFast24(x, y);
                                  inc(yy, 4);
                                end;
                              end;
                            end
