...
uses
  Windows, Classes, SysUtils, Messages, MMSystem, se_DXDUtils;
...
begin
  if b then
    Exit;
  raise Exception.Create(Format(Fmt, A));
end;
...
begin
  ...
  begin
    ...
    begin
      raise Exception.Create('TMidi.LoadData(): Error in MIDI data');
      Result := False;
      Exit;
    end;
  end;
...
begin
  ...
  FreeMem(MidiData, MidiDataSize);

  midi := TMidi.Create();

  f := TFileStream.Create(aMidiFile, fmOpenRead or fmShareDenyWrite);
  MidiDataSize := f.Size;
  GetMem(MidiData, MidiDataSize);
...
begin
  if midi <> nil then
    midi.SetVolume(midivolumecontrol[GetIntegerInRange(volume, 0, 15)]);
end;
