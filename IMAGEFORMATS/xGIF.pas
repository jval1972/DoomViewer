...
begin
  ...
  if LogicalScreen.PackedFields and lsdGlobalColorTable = lsdGlobalColorTable
    then
  begin
    TableSize := Trunc(intPower(2, (LogicalScreen.PackedFields and
      lsdColorTableSize) + 1));
    FStream.Read(GlobalColorTable, TableSize * SizeOf(TColorItem));
  ...
  FStream.Read(ImageDescriptor, SizeOf(ImageDescriptor));
    { read image descriptor }
  {Decode image header info}
  if ImageDescriptor.Separator <> ImageSeparator then
  ...
  begin { if local color table }
    TableSize := Trunc(intPower(2, (ImageDescriptor.PackedFields and
      idColorTableSize) + 1));
    FStream.Read(LocalColorTable, TableSize * SizeOf(TColorItem));
...
begin
  FStream.Read(ImageData.Size, SizeOf(ImageData.Size));
...
begin
  ...
  begin
    ...
    biCompression := BI_RGB;
      {Sorry Did not implement compression in this version}
